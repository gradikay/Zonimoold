...
//class Dogs_model extends CI_Model{
    ...
    public function report(){
        ...
        $animal_id      = $this->input->post('f0098');
        $animal_name    = $this->input->post('x0098');
        $section_id     = $this->input->post('i0078');
        $specie_id      = $this->input->post('o007');
        $sub_specie_id  = $this->input->post('d666');
        ...
        $sql = "INSERT INTO report_factopedia (username , report, reported_id, reported_animal_name, section_id, specie_id, sub_specie_id, report_message)            
                VALUES (" . $this->db->escape($username ) . ",
                        " . $this->db->escape($report) . ",
                        " . $this->db->escape($animal_id) . ",
                        " . $this->db->escape($animal_name) . ",
                        " . $this->db->escape($section_id) . ",
                        " . $this->db->escape($specie_id) . ",
                        " . $this->db->escape($sub_specie_id) . ",
     ...
     public function fetch_cats($limit, $start) { 
        ...
        $this->db->select('*') ;
        $this->db->from('cats AS D') ;
        $this->db->order_by("name", "asc");
        $this->db->join('cats_first_image  AS F', 'D.name = F.animal_name','left') ;
        $this->db->join('cats_second_image AS S', 'D.name = S.animal_name','left') ;
        $this->db->join('cats_third_image  AS T', 'D.name = T.animal_name','left') ;
        $this->db->join('cats_fourth_image AS R', 'D.name = R.animal_name','left') ;
        $query = $this->db->get();
 ...
 */
    ...
    public function get_cat_databases(){
        $this->db->select('*');
        $this->db->from('section AS S');
        $this->db->join('cats AS D', 'S.section_id = D.section_id', 'INNER');
...
//Update the animal data selected here we have the cats' table
    
     public function update($id){
          // This is the setting for the editing part of cats' page
          // OLD CODE
        /* $data = array(
            ...
            $coat         = $this->input->post('nkTdZQcYAo');
    ...
    public function cats_do_upload_first(){
    ...
    public function cats_do_upload_second(){
        $username = $this->session->userdata('username');
        // You can call any of this items using the cariable 
            // $upload_data
			$data = array('upload_data' => $this->upload->data());
    ...
    public function cats_do_upload_third(){
        $username = $this->session->userdata('username');
        // You can call any of this items using the cariable 
            // $upload_data
			$data = array('upload_data' => $this->upload->data());
    ...
    public function cats_do_upload_fourth(){
        $username = $this->session->userdata('username');
        // You can call any of this items using the cariable 
            // $upload_data
			$data = array('upload_data' => $this->upload->data());
