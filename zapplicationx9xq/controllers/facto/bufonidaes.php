...
class Bufonidaes extends CI_Controller {  
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Bufonidaes_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Bufonidaes/bufonidaes';
...
/*
 *
 * ************* This is the main page bufonidae' settings 
 *
 */    
    public function bufonidaes(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Bufonidaes/bufonidaes';
            // This gets the total row from my dogs' table pasted from dog's controller
        $config['total_rows']  = $this->Bufonidaes_model->record_count();
        ...
        $data['results']      = $this->Bufonidaes_model->fetch_bufonidaes($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Bufonidaes_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/bufonidaes';
        $this->load->view('layouts/main',$data);
    }
    
/*
 *
 * ************* This is the edit page bufonidae' settings 
 * ************* This id is returning all the data with id request
 */   
    public function bufonidae_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Bufonidaes_model->get_bufonidae_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Bufonidaes_model->get_users();
        $data['image']        = $this->Bufonidaes_model->get_my_images($id);

		$data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Bufonidaes_model->get_bufonidae_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/bufonidae_e';
...
/*
 *
 * ************* This is the more page bufonidae' settings 
 * ************* This id is returning all the data with id request
 */
    public function bufonidae_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Bufonidaes_model->get_bufonidae_details($id);
        $data['main_content'] = 'pagefactopedia/learn/bufonidae_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
...
/*
 *
 * ************* This is the first image model for the bufonidae's page 
 * ************* This id is returning all the data with id request
 */
    public function bufonidae_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/bufonidae_i',$data);
	}
    
/*
 *
 * ************* This is the second image model for the bufonidae's page 
 * ************* This id is returning all the data with id request
 */
    public function bufonidae_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/bufonidae_i2',$data);
	}
    
/*
 *
 * ************* This is the third image model for the bufonidae's page 
 * ************* This id is returning all the data with id request
 */
    public function bufonidae_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/bufonidae_i3',$data);
	}
    
/*
 *
 * ************* This is the fourth image model for the bufonidae's page 
 * ************* This id is returning all the data with id request
 */
    public function bufonidae_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/bufonidae_i4',$data);
	} 
    
/*
 *
 * ************* This is the Update the information in the more page for bufonidae
 * ************* This id is returning all the data with id request
 */
     public function fastmha(){          
        ...
        $this->Bufonidaes_model->update($id);
         
         $this->session->set_flashdata('failed_success','Thank you for updating factopedia. Your contribution is highly appreciate');
         
            // If success redirect to Bufonidaes/bufonidaes.php 
        redirect($this->redirecteur);
            
     }
/*
 *
 * ************* This is the upload the first image in the flexslider on the bufonidae's page 
 * ************* This id is returning all the data with id request
 */    
    public function bufonidaes_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Bufonidaes_model->bufonidaes_do_upload();
            $this->Bufonidaes_model->bufonidaes_do_upload_first();
...
/*
 *
 * ************* This is the upload the second image in the flexslider on the bufonidae's page 
 * ************* This id is returning all the data with id request
 */ 
    public function bufonidaes_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Bufonidaes_model->bufonidaes_do_upload();
            $this->Bufonidaes_model->bufonidaes_do_upload_second();
...
/*
 *
 * ************* This is the upload the third image in the flexslider on the bufonidae's page 
 * ************* This id is returning all the data with id request
 */    
    public function bufonidaes_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Bufonidaes_model->bufonidaes_do_upload();
            $this->Bufonidaes_model->bufonidaes_do_upload_third();
...
/*
 *
 * ************* This is the upload the fourth image in the flexslider on the bufonidae's page 
 * ************* This id is returning all the data with id request
 */ 
    public function bufonidaes_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Bufonidaes_model->bufonidaes_do_upload();
            $this->Bufonidaes_model->bufonidaes_do_upload_fourth();
