...
class Cetaceas extends CI_Controller { 
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Cetaceas_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Cetaceas/cetaceas';
...
/*
 *
 * ************* This is the main page cetacea' settings 
 *
 */    
    public function cetaceas(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Cetaceas/cetaceas';
            // This gets the total row from my dogs' table
        $config['total_rows']  = $this->Cetaceas_model->record_count();
        ...
        $data['results']      = $this->Cetaceas_model->fetch_cetaceas($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Cetaceas_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/cetaceas';
        $this->load->view('layouts/main',$data);
    }
    
/*
 *
 * ************* This is the edit page cetacea' settings 
 * ************* This id is returning all the data with id request
 */   
    public function cetacea_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Cetaceas_model->get_cetacea_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Cetaceas_model->get_users();
        $data['image']        = $this->Cetaceas_model->get_my_images($id);

		$data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Cetaceas_model->get_cetacea_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/cetacea_e';
...
/*
 *
 * ************* This is the more page cetacea' settings 
 * ************* This id is returning all the data with id request
 */
    public function cetacea_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Cetaceas_model->get_cetacea_details($id);
        $data['main_content'] = 'pagefactopedia/learn/cetacea_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
...
/*
 *
 * ************* This is the first image model for the cetacea's page 
 * ************* This id is returning all the data with id request
 */
    public function cetacea_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/cetacea_i',$data);
	}
    
/*
 *
 * ************* This is the second image model for the cetacea's page 
 * ************* This id is returning all the data with id request
 */
    public function cetacea_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/cetacea_i2',$data);
	}
    
/*
 *
 * ************* This is the third image model for the cetacea's page 
 * ************* This id is returning all the data with id request
 */
    public function cetacea_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/cetacea_i3',$data);
	}
    
/*
 *
 * ************* This is the fourth image model for the cetacea's page 
 * ************* This id is returning all the data with id request
 */
    public function cetacea_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/cetacea_i4',$data);
	} 
    
/*
 *
 * ************* This is the Update the information in the more page for cetacea
 * ************* This id is returning all the data with id request
 */
     public function fastmha(){          
        ...
        $this->Cetaceas_model->update($id);
         
         $this->session->set_flashdata('failed_success','Thank you for updating factopedia. Your contribution is highly appreciate');
         
            // If success redirect to Cetaceas/cetaceas.php
        redirect($this->redirecteur);
            
     }
/*
 *
 * ************* This is the upload the first image in the flexslider on the cetacea's page 
 * ************* This id is returning all the data with id request
 */    
    public function cetaceas_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Cetaceas_model->cetaceas_do_upload();
            $this->Cetaceas_model->cetaceas_do_upload_first();
...
/*
 *
 * ************* This is the upload the second image in the flexslider on the cetacea's page 
 * ************* This id is returning all the data with id request
 */ 
    public function cetaceas_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Cetaceas_model->cetaceas_do_upload();
            $this->Cetaceas_model->cetaceas_do_upload_second();
...
/*
 *
 * ************* This is the upload the third image in the flexslider on the cetacea's page 
 * ************* This id is returning all the data with id request
 */    
    public function cetaceas_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Cetaceas_model->cetaceas_do_upload();
            $this->Cetaceas_model->cetaceas_do_upload_third();
...
/*
 *
 * ************* This is the upload the fourth image in the flexslider on the cetacea's page 
 * ************* This id is returning all the data with id request
 */ 
    public function cetaceas_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Cetaceas_model->cetaceas_do_upload();
            $this->Cetaceas_model->cetaceas_do_upload_fourth();
