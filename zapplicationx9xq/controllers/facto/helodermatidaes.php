...
class Helodermatidaes extends CI_Controller { 
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Helodermatidaes_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Helodermatidaes/helodermatidaes';
 ...
 *
 */    
    public function helodermatidaes(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Helodermatidaes/helodermatidaes';
            // This gets the total row from my dogs' table pasted from dog's controller
        $config['total_rows']  = $this->Helodermatidaes_model->record_count();
        ...
        $data['results']      = $this->Helodermatidaes_model->fetch_helodermatidaes($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Helodermatidaes_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/helodermatidaes';
 ...
 */   
    public function helodermatidae_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Helodermatidaes_model->get_helodermatidae_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Helodermatidaes_model->get_users();
        $data['image']        = $this->Helodermatidaes_model->get_my_images($id);

		$data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Helodermatidaes_model->get_helodermatidae_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/helodermatidae_e';
 ...
 * ************* This id is returning all the data with id request
 */
    public function helodermatidae_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Helodermatidaes_model->get_helodermatidae_details($id);
        $data['main_content'] = 'pagefactopedia/learn/helodermatidae_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
 ...
 * ************* This id is returning all the data with id request
 */
    public function helodermatidae_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/helodermatidae_i',$data);
	}
 ...
 * ************* This id is returning all the data with id request
 */
    public function helodermatidae_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/helodermatidae_i2',$data);
	}
 ...
 * ************* This id is returning all the data with id request
 */
    public function helodermatidae_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/helodermatidae_i3',$data);
	}
 ...
 * ************* This id is returning all the data with id request
 */
    public function helodermatidae_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/helodermatidae_i4',$data);
	} 
 ...
 * ************* This id is returning all the data with id request
 */    
    public function helodermatidaes_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Helodermatidaes_model->helodermatidaes_do_upload();
            $this->Helodermatidaes_model->helodermatidaes_do_upload_first();
 ...
 * ************* This id is returning all the data with id request
 */ 
    public function helodermatidaes_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Helodermatidaes_model->helodermatidaes_do_upload();
            $this->Helodermatidaes_model->helodermatidaes_do_upload_second();
 ...
 * ************* This id is returning all the data with id request
 */    
    public function helodermatidaes_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Helodermatidaes_model->helodermatidaes_do_upload();
            $this->Helodermatidaes_model->helodermatidaes_do_upload_third();
 ...
 * ************* This id is returning all the data with id request
 */ 
    public function helodermatidaes_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Helodermatidaes_model->helodermatidaes_do_upload();
            $this->Helodermatidaes_model->helodermatidaes_do_upload_fourth();
