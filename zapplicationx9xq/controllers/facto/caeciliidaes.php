...
class Caeciliidaes extends CI_Controller { 
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Caeciliidaes_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Caeciliidaes/caeciliidaes';
...
/*
 *
 * ************* This is the main page caeciliidae' settings 
 *
 */    
    public function caeciliidaes(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Caeciliidaes/caeciliidaes';
            // This gets the total row from my dogs' table pasted from dog's controller
        $config['total_rows']  = $this->Caeciliidaes_model->record_count();
        ...
        $data['results']      = $this->Caeciliidaes_model->fetch_caeciliidaes($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Caeciliidaes_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/caeciliidaes';
        $this->load->view('layouts/main',$data);
    }
    
/*
 *
 * ************* This is the edit page caeciliidae' settings 
 * ************* This id is returning all the data with id request
 */   
    public function caeciliidae_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Caeciliidaes_model->get_caeciliidae_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Caeciliidaes_model->get_users();
        $data['image']        = $this->Caeciliidaes_model->get_my_images($id);

		$data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Caeciliidaes_model->get_caeciliidae_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/caeciliidae_e';
...
/*
 *
 * ************* This is the more page caeciliidae' settings 
 * ************* This id is returning all the data with id request
 */
    public function caeciliidae_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Caeciliidaes_model->get_caeciliidae_details($id);
        $data['main_content'] = 'pagefactopedia/learn/caeciliidae_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
...
/*
 *
 * ************* This is the first image model for the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */
    public function caeciliidae_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/caeciliidae_i',$data);
	}
    
/*
 *
 * ************* This is the second image model for the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */
    public function caeciliidae_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/caeciliidae_i2',$data);
	}
    
/*
 *
 * ************* This is the third image model for the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */
    public function caeciliidae_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/caeciliidae_i3',$data);
	}
    
/*
 *
 * ************* This is the fourth image model for the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */
    public function caeciliidae_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/caeciliidae_i4',$data);
	} 
    
/*
 *
 * ************* This is the Update the information in the more page for caeciliidae
 * ************* This id is returning all the data with id request
 */
     public function fastmha(){          
        ...
        $this->Caeciliidaes_model->update($id);
         
         $this->session->set_flashdata('failed_success','Thank you for updating factopedia. Your contribution is highly appreciate');
         
            // If success redirect to Caeciliidaes/caeciliidaes.php
        redirect($this->redirecteur);
            
     }
/*
 *
 * ************* This is the upload the first image in the flexslider on the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */    
    public function caeciliidaes_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Caeciliidaes_model->caeciliidaes_do_upload();
            $this->Caeciliidaes_model->caeciliidaes_do_upload_first();
...
/*
 *
 * ************* This is the upload the second image in the flexslider on the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */ 
    public function caeciliidaes_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Caeciliidaes_model->caeciliidaes_do_upload();
            $this->Caeciliidaes_model->caeciliidaes_do_upload_second();
...
/*
 *
 * ************* This is the upload the third image in the flexslider on the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */    
    public function caeciliidaes_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Caeciliidaes_model->caeciliidaes_do_upload();
            $this->Caeciliidaes_model->caeciliidaes_do_upload_third();
...
/*
 *
 * ************* This is the upload the fourth image in the flexslider on the caeciliidae's page 
 * ************* This id is returning all the data with id request
 */ 
    public function caeciliidaes_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Caeciliidaes_model->caeciliidaes_do_upload();
            $this->Caeciliidaes_model->caeciliidaes_do_upload_fourth();
