...
class Chiropteras extends CI_Controller { 
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Chiropteras_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Chiropteras/chiropteras';
...
/*
 *
 * ************* This is the main page chiroptera' settings 
 *
 */    
    public function chiropteras(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Chiropteras/chiropteras';
            // This gets the total row from my dogs' table pasted from chiroptera's controller
        $config['total_rows']  = $this->Chiropteras_model->record_count();
        ...
        $data['results']      = $this->Chiropteras_model->fetch_chiropteras($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Chiropteras_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/chiropteras';
        $this->load->view('layouts/main',$data);
    }
    
/*
 *
 * ************* This is the edit page chiroptera' settings 
 * ************* This id is returning all the data with id request
 */   
    public function chiroptera_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Chiropteras_model->get_chiroptera_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Chiropteras_model->get_users();
        $data['image']        = $this->Chiropteras_model->get_my_images($id);

		$data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Chiropteras_model->get_chiroptera_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/chiroptera_e';
...
/*
 *
 * ************* This is the more page chiroptera' settings 
 * ************* This id is returning all the data with id request
 */
    public function chiroptera_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Chiropteras_model->get_chiroptera_details($id);
        $data['main_content'] = 'pagefactopedia/learn/chiroptera_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
...
/*
 *
 * ************* This is the first image model for the chiroptera's page 
 * ************* This id is returning all the data with id request
 */
    public function chiroptera_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/chiroptera_i',$data);
	}
    
/*
 *
 * ************* This is the second image model for the chiroptera's page 
 * ************* This id is returning all the data with id request
 */
    public function chiroptera_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/chiroptera_i2',$data);
	}
    
/*
 *
 * ************* This is the third image model for the chiroptera's page 
 * ************* This id is returning all the data with id request
 */
    public function chiroptera_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/chiroptera_i3',$data);
	}
    
/*
 *
 * ************* This is the fourth image model for the chiroptera's page 
 * ************* This id is returning all the data with id request
 */
    public function chiroptera_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/chiroptera_i4',$data);
	} 
    
/*
 *
 * ************* This is the Update the information in the more page for chiroptera
 * ************* This id is returning all the data with id request
 */
     public function fastmha(){          
        ...
        $this->Chiropteras_model->update($id);
         
         $this->session->set_flashdata('failed_success','Thank you for updating factopedia. Your contribution is highly appreciate');
         
            // If success redirect to Chiropteras/chiropteras.php
        redirect($this->redirecteur);
            
     }
/*
 *
 * ************* This is the upload the first image in the flexslider on the chiroptera's page 
 * ************* This id is returning all the data with id request
 */    
    public function chiropteras_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Chiropteras_model->chiropteras_do_upload();
            $this->Chiropteras_model->chiropteras_do_upload_first();
...
/*
 *
 * ************* This is the upload the second image in the flexslider on the chiroptera's page 
 * ************* This id is returning all the data with id request
 */ 
    public function chiropteras_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Chiropteras_model->chiropteras_do_upload();
            $this->Chiropteras_model->chiropteras_do_upload_second();
...
/*
 *
 * ************* This is the upload the third image in the flexslider on the chiroptera's page 
 * ************* This id is returning all the data with id request
 */    
    public function chiropteras_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Chiropteras_model->chiropteras_do_upload();
            $this->Chiropteras_model->chiropteras_do_upload_third();
...
/*
 *
 * ************* This is the upload the fourth image in the flexslider on the chiroptera's page 
 * ************* This id is returning all the data with id request
 */ 
    public function chiropteras_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Chiropteras_model->chiropteras_do_upload();
            $this->Chiropteras_model->chiropteras_do_upload_fourth();
