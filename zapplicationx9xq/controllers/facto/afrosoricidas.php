...
class Afrosoricidas extends CI_Controller { 
    ...
    public function __construct()
        {
				...
				$this->model = 'factopedia/Afrosoricidas_model';
					// Set the variable for $redirecteur above
				$this->redirecteur = 'facto/Afrosoricidas/afrosoricidas';
...
/*
 *
 * ************* This is the main page afrosoricida' settings 
 *
 */    
    public function afrosoricidas(){
         ...
         * ******************** Pagination setting
         */
        $config['base_url']    = base_url(). '/facto/Afrosoricidas/afrosoricidas';
            // This gets the total row from my dogs' table
        $config['total_rows']  = $this->Afrosoricidas_model->record_count();
        ...
        $data['results']      = $this->Afrosoricidas_model->fetch_afrosoricidas($config['per_page'], $page);
        ...
        $data['getAll']       = $this->Afrosoricidas_model->return_row_count();
		$data['getEverything']= $this->Count_model->return_all_database_count();
        $data['main_content'] = 'pagefactopedia/afrosoricidas';
        $this->load->view('layouts/main',$data);
    }
    
/*
 *
 * ************* This is the edit page afrosoricida' settings 
 * ************* This id is returning all the data with id request
 */   
    public function afrosoricida_e($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $this->load->model($this->model);
             ...
             */
        $data['lists']        = $this->Afrosoricidas_model->get_afrosoricida_databases(); 
            // Call the users variable and the table name to output the result
            // Call it everywhere you want to use it 
        $data['users']        = $this->Afrosoricidas_model->get_users();
        $data['image']        = $this->Afrosoricidas_model->get_my_images($id);

        $data['im_prof']      = $this->User_model->get_im_profile();
        $data['spons']        = $this->Buyorsell_model->pets_sponsord();
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Afrosoricidas_model->get_afrosoricida_details($id);
        $data['main_content'] = 'pagefactopedia/editorial/afrosoricida_e';
...
/*
 *
 * ************* This is the more page afrosoricida' settings 
 * ************* This id is returning all the data with id request
 */
    public function afrosoricida_m($id){
        // To simplify thing we are taking the ads or sponsorized items from the buyorsell model 
        $this->load->model($this->buyorsell_model);
        $this->load->helper('text');
        ...
        $data['sponsf']       = $this->Buyorsell_model->pets_sponsordfoot();
        $data['animal']       = $this->Afrosoricidas_model->get_afrosoricida_details($id);
        $data['main_content'] = 'pagefactopedia/learn/afrosoricida_m';
            /*
             * this will iclude the main.php inside layouts
             */
        $this->load->view('layouts/main',$data);
...
/*
 *
 * ************* This is the first image model for the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */
    public function afrosoricida_i($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/afrosoricida_i',$data);
	}
    
/*
 *
 * ************* This is the second image model for the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */
    public function afrosoricida_i2($id){
            // Loads the model
        $this->load->model($this->model);
        ...
        $this->load->view('pagefactopedia/modelsimage/afrosoricida_i2',$data);
	}
    
/*
 *
 * ************* This is the third image model for the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */
    public function afrosoricida_i3($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/afrosoricida_i3',$data);
	}
    
/*
 *
 * ************* This is the fourth image model for the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */
    public function afrosoricida_i4($id){
        ...
        $this->load->view('pagefactopedia/modelsimage/afrosoricida_i4',$data);
	} 
    
/*
 *
 * ************* This is the Update the information in the more page for afrosoricida 
 * ************* This id is returning all the data with id request
 */
     public function fastmha(){          
        ...
        $this->Afrosoricidas_model->update($id);
         
         $this->session->set_flashdata('failed_success','Thank you for updating factopedia. Your contribution is highly appreciate');
         
            // If success redirect to Afrosoricidas/afrosoricidas.php
        redirect($this->redirecteur);
            
     }
/*
 *
 * ************* This is the upload the first image in the flexslider on the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */    
    public function afrosoricidas_do_upload(){ 
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->Afrosoricidas_model->afrosoricidas_do_upload();
            $this->Afrosoricidas_model->afrosoricidas_do_upload_first();
...
/*
 *
 * ************* This is the upload the second image in the flexslider on the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */ 
    public function afrosoricidas_do_upload2(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Afrosoricidas_model->afrosoricidas_do_upload();
            $this->Afrosoricidas_model->afrosoricidas_do_upload_second();
...
/*
 *
 * ************* This is the upload the third image in the flexslider on the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */    
    public function afrosoricidas_do_upload3(){      
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Afrosoricidas_model->afrosoricidas_do_upload();
            $this->Afrosoricidas_model->afrosoricidas_do_upload_third();
...
/*
 *
 * ************* This is the upload the fourth image in the flexslider on the afrosoricida's page 
 * ************* This id is returning all the data with id request
 */ 
    public function afrosoricidas_do_upload4(){  
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $this->image_lib->clear();
                ...
                //This two will be executed at the sametime
            $this->Afrosoricidas_model->afrosoricidas_do_upload();
            $this->Afrosoricidas_model->afrosoricidas_do_upload_fourth();
